...
async function main() {
  ...
  const [s3Bucket, s3KeyPrefix] = destPath.split('/');
  if (!s3Bucket) {
  ...
  for (const filePath of allFiles) {
    ...
    uploadRequests.push({
      ...
      CacheControl: hashedFileExtensions.includes(ext) ? CACHE_CONTROL_IMMUTABLE : CACHE_CONTROL_NO_CACHE,
    });
